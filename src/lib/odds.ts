...
export function getOddsError(raw: any, signToggleValue: string = "+"): string | null {
  ...
  // Check for negative decimals (but not American float odds like -118.5)
  if (t.startsWith('-') && t.includes('.')) {
    // Check if it's a valid American float odds (magnitude >= 100)
    const val = parseFloat(t);
    if (val < 0 && val > -100) {
      return "Negative decimal is invalid";
    }
  }
